...
export function parseEntitlementError(error: unknown): EntitlementError | null {
  ...
  // Check for entitlement error codes matching backend EntitlementException
  ...
  if (
    originalError.error === "quota_exceeded" &&
    originalError.details?.feature === "storage_mb"
  ) {
    return {
      code: "QUOTA_EXCEEDED",
...
export function isStorageQuotaError(error: unknown): boolean {
  ...
  return (
    ...
    parsed?.details?.feature === "storage_mb"
  );
}
 ...
 */
export function parseStorageQuotaError(
  error: unknown,
): StorageQuotaError["details"] | null {
  if (!(error instanceof ApiClientError)) {
    return null;
  }
